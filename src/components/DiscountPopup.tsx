...
export function DiscountPopup() {
  ...
  useEffect(() => {
    const sync = () => {
      const offerUnlocked = isOfferUnlocked();
      setUnlocked(offerUnlocked);
      try {
        const launcherGone =
          sessionStorage.getItem(OFFER_LAUNCHER_DISMISSED_KEY) === "1";
        setShowLauncher(!offerUnlocked && !launcherGone);
      } catch {
        setShowLauncher(!offerUnlocked);
      }
    };
    sync();
    window.addEventListener(OFFER_UNLOCKED_EVENT, sync);
    return () => window.removeEventListener(OFFER_UNLOCKED_EVENT, sync);
  }, []);
  ...
  useEffect(() => {
    const onOpen = () => {
      setStep(isOfferUnlocked() ? "reveal" : "pick");
      setOpen(true);
      try {
        sessionStorage.removeItem(OFFER_DISMISSED_KEY);
      } catch {
        /* ignore */
      }
    };
  ...
  function dismissLauncher(e: MouseEvent) {
    e.stopPropagation();
    try {
      sessionStorage.setItem(OFFER_LAUNCHER_DISMISSED_KEY, "1");
    } catch {
      /* ignore */
    }
    setShowLauncher(false);
    ...
    <>
      {showLauncher && !open ? (
        <button
          type="button"
          onClick={() => {
            setStep(unlocked ? "reveal" : "pick");
            setOpen(true);
          }}
          className="fixed bottom-4 left-4 z-[60] inline-flex items-center gap-2 rounded-md bg-purple px-4 py-2.5 font-display text-xs font-bold uppercase tracking-wide text-white shadow-brand hover:bg-purple-deep"
        >
          Special Discount
          <span
            role="button"
            ...
            className="inline-grid h-5 w-5 place-items-center rounded bg-white/15 text-sm"
          >
            ×
          </span>
        </button>
                  ...
                  {[0, 1, 2].map((i) => (
                    <button
                      ...
                      className={`aspect-square rounded-xl bg-[linear-gradient(160deg,#9b5ec4_0%,#6b2d91_55%,#4a1d68_100%)] font-display text-[clamp(2rem,8vw,2.6rem)] font-extrabold text-white shadow-[0_14px_30px_-16px_rgba(26,18,37,0.55)] transition hover:-translate-y-0.5 ${
                        selectedBox === i
                          ? "animate-pulse outline outline-2 outline-offset-2 outline-white"
                          : ""
                      }`}
                    >
                      ?
            ...
            {step === "email" && (
              <div>
                ...
                <form onSubmit={onSubmit} className="mb-4 grid gap-3 text-left">
                  ...
                  <p className="text-center text-xs leading-snug text-white/80">
                    Enter your email to unlock the Pre-Launch Bundle deal.
                    Unsubscribe anytime.
                  </p>
                  {error ? (
                    <p className="text-center text-sm text-[#ffe0e0]">{error}</p>
                  ) : null}
                </form>
                <div className="-mx-5 overflow-hidden rounded-b-2xl">
                  <Image
